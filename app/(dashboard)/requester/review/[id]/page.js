...
export default function SafeRequesterReviewPage({ params }) {
  const router = useRouter();
  
  // Dynamic extraction layout engine wrapper
  let requisitionId = null;
  if (params) {
    if (typeof params.then === 'function') {
      try {
        const unwrapped = use(params);
        requisitionId = unwrapped?.id;
      } catch (e) {
        console.error("Async parameter extraction fault:", e);
      }
    } else {
  ...
  const handleSendMessage = async (e) => {
    ...
    // ✨ OPTIMISTIC UPDATE: Lock straight onto screen instantly
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-7 bg-white border border-[#E5E7EB] rounded-xl p-6 sm:p-8 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-[#E5E7EB] pt-6 text-xs font-semibold text-gray-500">
              <div className="p-3 bg-gray-50 rounded-lg"><div className="text-[10px] text-gray-400 uppercase">issuing operator</div><div className="text-sm font-bold text-gray-900 lowercase">{activeRecord.requester}</div></div>
              <div className="p-3 bg-gray-50 rounded-lg"><div className="text-[10px] text-gray-400 uppercase">deployment regional location</div><div className="text-sm font-bold text-gray-900 lowercase">{activeRecord.location || 'harare hub'}</div></div>
              <div className="p-3 bg-gray-50 rounded-lg"><div className="text-[10px] text-gray-400 uppercase">budget category tracking</div><div className="text-sm font-bold text-gray-900 lowercase">{activeRecord.category}</div></div>
              <div className="p-3 bg-blue-50 border border-blue-100 rounded-lg"><div className="text-[10px] text-[#0747A1] uppercase">aggregated fund volume (usd)</div><div className="text-base font-black text-gray-900 mt-0.5">${parseFloat(activeRecord.amount || 0).toFixed(2)}</div></div>
            </div>
